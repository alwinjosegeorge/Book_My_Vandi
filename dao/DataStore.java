...
public class DataStore {
    ...
    static {
        try {
            ...
            // Initialization is now manual (via signup)
            if (getUsersCount() == 0) {
                 // You must sign up for an admin/seller/user account first.
            }
...
public static class DashboardPanel extends JPanel {
    ...
    public DashboardPanel(MainFrame parent){
        ...
        tabs.setBackground(BACKGROUND_LIGHT); // Set tab background
    ...
    // FIX: Scoping error fix applied here
...
public CardBrowsePanel(MainFrame parent){
    ...
    header.setBackground(NAVY_HEADER);
    ...
    btnSearch.setBackground(PRIMARY_BLUE); btnSearch.setForeground(Color.WHITE);
...
public void refresh(){
    ...
    for(Car c : DataStore.fetchCars()){
        ...
        lblRating.setForeground(ACCENT_ORANGE); 
        info.add(lblRating);
        ...
        lblStatus.setForeground("Available".equalsIgnoreCase(c.status) ? ACCENT_GREEN : ACCENT_RED);
        info.add(lblStatus);
        ...
        if(currentUser.role.equals("user")){
            ...
            bookBtn.setBackground(PRIMARY_BLUE); bookBtn.setForeground(Color.WHITE);
...
public BookingDialog(MainFrame parent, Car car, User user){
    ...
    setLayout(new BorderLayout(15, 15)); // Use BorderLayout for the main structure
    getRootPane().setBorder(new EmptyBorder(15, 15, 15, 15));
    
    // --- 1. CAR DETAILS PANEL (West) ---
    JPanel carInfoPanel = new JPanel(new BorderLayout(10, 10));
    carInfoPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(CARD_BORDER), "Vehicle Details"));
    carInfoPanel.setBackground(BACKGROUND_LIGHT);
    
    // Car Image
    JLabel imgLabel = new JLabel();
    imgLabel.setHorizontalAlignment(SwingConstants.CENTER);
    imgLabel.setIcon(UIUtils.loadScaledIcon(car.imagePath, 300, 180));
    carInfoPanel.add(imgLabel, BorderLayout.NORTH);
    
    // Features List
    JPanel featuresPanel = new JPanel(new GridLayout(0, 1));
    featuresPanel.setBackground(BACKGROUND_LIGHT);
    featuresPanel.setBorder(new EmptyBorder(10, 5, 5, 5));
    featuresPanel.add(new JLabel("<html><b>Category:</b> " + car.category + "</html>"));
    featuresPanel.add(new JLabel("<html><b>Price/Day:</b> <span style='color: " + String.format("#%06x", ACCENT_GREEN.getRGB() & 0xFFFFFF) + ";'>₹" + UIUtils.MONEY.format(car.pricePerDay) + "</span></html>"));
    ...
    // --- 2. BOOKING INPUT PANEL (Center) ---
    JPanel inputPanel = new JPanel(new GridBagLayout());
    inputPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(CARD_BORDER), "Booking Information"));
    ...
    // Pickup Place
    gbc.gridx=0; gbc.gridy=0; inputPanel.add(new JLabel("Pickup Location:"),gbc);
    gbc.gridx=1; 
    JTextField txtPlace=new JTextField(car.category+" Rent Location", 25); 
    inputPanel.add(txtPlace,gbc);

    // Pickup Date
    gbc.gridx=0; gbc.gridy=1; inputPanel.add(new JLabel("Pickup Date (dd/MM/yyyy):"),gbc);
    gbc.gridx=1; 
    JTextField txtDate=new JTextField(new SimpleDateFormat("dd/MM/yyyy").format(new java.util.Date())); 
    inputPanel.add(txtDate,gbc);

    // Pickup Time
    gbc.gridx=0; gbc.gridy=2; inputPanel.add(new JLabel("Pickup Time (HH:mm):"),gbc);
    gbc.gridx=1; 
    JTextField txtTime=new JTextField(new SimpleDateFormat("HH:mm").format(new java.util.Date())); 
    inputPanel.add(txtTime,gbc);

    // No. of Days
    gbc.gridx=0; gbc.gridy=3; inputPanel.add(new JLabel("No. of Days:"),gbc);
    gbc.gridx=1; 
    JSpinner spinDays = new JSpinner(new SpinnerNumberModel(1,1,30,1)); 
    inputPanel.add(spinDays,gbc);
    ...
    lblTotal.setForeground(NAVY_HEADER);
    inputPanel.add(lblTotal,gbc);
    ...
    confirm.setBackground(ACCENT_GREEN); confirm.setForeground(Color.WHITE);
    ...
    // Panel to hold button (to align it right)
...
public static class SellerPanel extends JPanel {
    ...
    public SellerPanel(MainFrame parent){
        ...
        tbl.getTableHeader().setBackground(NAVY_HEADER);
        tbl.getTableHeader().setForeground(Color.WHITE);
        tbl.setRowHeight(25);
        add(new JScrollPane(tbl),BorderLayout.CENTER);

        JPanel bottom = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        bottom.setBackground(BACKGROUND_LIGHT);
...
public static class BookingHistoryPanel extends JPanel {
    ...
    public BookingHistoryPanel(MainFrame parent){
        ...
        tbl.getColumnModel().getColumn(3).setCellRenderer(new StatusCellRenderer());
    }
    ...
    private void handleBookingAction(String action) {
        ...
        if (action.equals("Confirmed")) {
            ...
            if (currentCar != null) {
             ...
             Car currentCar = DataStore.findCarById(carId); 
             if (currentCar != null && currentCar.status.equals("Rented")) {
                 currentCar.status = "Available"; // If rejected, return car to available status
                 DataStore.updateCar(currentCar);
             }
        }
